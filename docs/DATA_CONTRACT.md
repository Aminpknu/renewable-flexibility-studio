...
`data/latest_spatial_demand_forecast.csv` is the companion ten-zone **underlying-demand proxy** for the same target date. DESNZ 2024 Local Authority electricity consumption sets annual zone weights; Elexon CDCA-I029 Aggregated GSP Group Take archives provide regional within-day profiles; NESO GSP-region boundaries map Local Authorities to GSP Groups. `data/spatial_demand_zone_weights.csv`, `data/spatial_demand_zone_gsp_mix.csv` and `data/gsp_group_demand_profiles.csv` retain the derived weighting/profile evidence.

National Demand Forecast is not treated as gross customer demand because embedded wind/solar suppress it. The bundle therefore records `national_underlying_demand_proxy_mw = national_demand_mw + national_embedded_wind_solar_mw`, then allocates that underlying proxy across ten zones. Required zone fields include `zone_underlying_demand_mw`, `zone_demand_share`, annual consumption weight and pre-delivery NDF publication time. Ten zone underlying-demand values must sum to the national underlying proxy each period. When joined to `latest_spatial_forecast.csv`, subtracting the identical zone embedded wind/solar allocation must make the ten zone net loads sum back to NESO National Demand. `data/spatial_demand_manifest.json` locks sources, checksums, validation evidence and this semantic boundary.

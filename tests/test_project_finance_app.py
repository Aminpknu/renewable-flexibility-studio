...
def test_project_finance_ui_uses_stage10_base_and_labels_stage11_upside() -> None:
    ...
    assert "Base project NPV" in labels
    assert "Minimum DSCR" in labels
    assert "Stage 11 upside project NPV" in labels
    assert "bankable debt-service revenue" in str(note)
...
def test_project_finance_monte_carlo_ui_and_download() -> None:
    ...
    assert "Stage 11 ancillary-service upside is excluded" in str(note)
    ...
    assert any("perfect-information" in text for text in body["boundaries"])
